<head>
  ...
  <style>
    ...
    button.premik_svetlo {background-color: 60C040;}
    button.premik_temno {background-color: 48A838;}
    button.premik_svetlo:hover {background-color: 40C030;}
    button.premik_temno:hover {background-color: 20A814;}
  ...
  %for k in range(8):
    ...
    %elif (k, l) in veljavni:
      ...
      <input type="hidden", name="premiki", value="{{i}}{{j}}{{k}}{{l}}">
        <td>
          <button type="submit", class="premik_{{polje}}">
          <p class="{{sah.barva_igra(k, l)}}">{{sah.figura_igra(k, l)}}</p>
          </button>
        </td>
      </form>
